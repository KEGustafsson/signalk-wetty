...
| WeTTY log level              | `silent`            | `silent` keeps WeTTY out of the Signal K server log entirely. Any other level (`error` … `silly`) writes WeTTY's connection and request logging to the server console.                                 |
